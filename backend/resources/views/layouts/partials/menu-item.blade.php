@props(['route', 'routeName', 'icon', 'label', 'color' => 'gray', 'placeholder' => false])

@php
$isActive = request()->routeIs($route);
$hasRoute = !$placeholder && Route::has($routeName);
...
$gradient = $colors[$color] ?? $colors['gray'];
@endphp

<li>
    <a href="{{ $href }}" 
       @if($placeholder) title="Coming Soon" @endif
       class="flex items-center p-2.5 rounded-lg transition-all group
          {{ $isActive 
             ? 'bg-gradient-to-r from-'.$color.'-600/20 to-'.$color.'-500/20 text-white border border-'.$color.'-500/30' 
             : 'text-gray-400 hover:bg-gray-700/50 hover:text-white' }}
          {{ $placeholder ? 'opacity-50 cursor-not-allowed' : '' }}">
        <div class="p-1.5 rounded-md bg-gradient-to-br {{ $gradient }} {{ $isActive ? '' : 'opacity-60 group-hover:opacity-100' }}">
            @switch($icon)
                @case('user-plus')
                    ...
                    @break
                @case('users')
                    <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20"><path d="M9 6a3 3 0 11-6 0 3 3 0 016 0zM17 6a3 3 0 11-6 0 3 3 0 016 0zM12.93 17c.046-.327.07-.66.07-1a6.97 6.97 0 00-1.5-4.33A5 5 0 0119 16v1h-6.07zM6 11a5 5 0 015 5v1H1v-1a5 5 0 015-5z"/></svg>
                    @break
                @case('document-text')
                ...
                @case('chart-bar')
                    ...
                    @break
                @case('server')
                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2m-2-4h.01M17 16h.01"/></svg>
                    @break
                @case('cube')
                ...
                @case('map-pin')
                    ...
                    @break
                @case('wifi')
                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.111 16.404a5.5 5.5 0 017.778 0M12 20h.01m-7.08-7.071c3.904-3.905 10.236-3.905 14.141 0M1.394 9.393c5.857-5.857 15.355-5.857 21.213 0"/></svg>
                    @break
                @case('globe')
                ...
                @case('tag')
                    ...
                    @break
                @case('gift')
                    <svg class="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20"><path d="M18 5h-.7c.229-.467.349-.98.351-1.5a3.5 3.5 0 0 0-3.5-3.5c-1.717 0-3.215 1.2-4.331 2.481C8.4.842 6.949 0 5.5 0A3.5 3.5 0 0 0 2 3.5c.003.52.123 1.033.351 1.5H2a2 2 0 0 0-2 2v3a1 1 0 0 0 1 1h18a1 1 0 0 0 1-1V7a2 2 0 0 0-2-2Z"/></svg>
                    @break
                @case('cog')
        ...
        @if($placeholder)
            <span class="ms-auto px-1.5 py-0.5 text-[10px] font-medium bg-gray-700 text-gray-400 rounded">Soon</span>
        @endif
    </a>
</li>
