    ...
    <div class="py-8">
        <div class="max-w-full sm:px-6 lg:px-8">
            ...
            @elseif($userRole === 'warehouse')
                ...
                <!-- Low Stock Alert List (Warehouse) -->
                <div class="rounded-2xl bg-gray-800 border border-gray-700/50 p-6">
                    <h3 class="text-lg font-bold text-white mb-4 flex items-center">
                        <svg class="w-5 h-5 mr-2 text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @if($lowStockList->count() > 0)
                        <div class="space-y-4">
                            @foreach ($lowStockList as $item)
                                <div class="flex items-center justify-between p-4 bg-gray-900/50 rounded-xl border border-red-500/20">
                                    <div>
                                        <p class="text-white font-medium">{{ $item->name }}</p>
                                        <p class="text-xs text-gray-500">SKU: {{ $item->sku ?: '-' }} | Kategori: {{ $item->category->name }}</p>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-xl font-bold text-red-400">{{ $item->total_stock }} {{ $item->unit }}</p>
                                        <p class="text-[10px] text-gray-500">Batas: {{ $item->min_stock_alert }}</p>
                                    </div>
                                </div>
                            @endforeach
                            <a href="{{ route('inventory.index') }}" class="block text-center text-sm text-amber-400 hover:text-amber-300 transition">Lihat Semua Inventaris →</a>
                        </div>
                    @else
                        <div class="text-center py-8">
                            <svg class="w-12 h-12 mx-auto text-emerald-500/30 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
