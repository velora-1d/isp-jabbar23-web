...
class TechnicianApiController extends Controller
{
    ...
    /**
     * Get work orders assigned to the authenticated technician.
     */
    public function myWorkOrders(Request $request)
    {
        $workOrders = WorkOrder::where('technician_id', Auth::id())
            ->with(['customer', 'items.inventoryItem', 'odp'])
            ->latest()
    ...
    /**
     * Update work order status from the field.
     */
    public function updateWorkOrderStatus(Request $request, WorkOrder $workOrder)
    {
        if ($workOrder->technician_id !== Auth::id()) {
            return response()->json(['error' => 'Unauthorized. Work order not assigned to you.'], 403);
        }

        $validated = $request->validate([
            'status' => 'required|in:on_way,in_progress,completed,cancelled',
            'notes'  => 'nullable|string',
            'photos' => 'nullable|array',
        ]);

        $workOrder = $this->workOrderService->updateStatus($workOrder, $validated['status'], $validated['notes'] ?? null);

        if (isset($validated['photos'])) {
            $workOrder->update(['photos' => $validated['photos']]);
        }
