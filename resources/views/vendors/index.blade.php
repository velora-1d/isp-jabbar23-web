...
@section('content')
    <div class="space-y-6">
        ...
        <x-filter-bar :filters="$filters ?? []">
            <x-slot name="global">
                <x-filter-global :search-placeholder="'Cari Vendor...'" />
            </x-slot>
            ...
            <x-slot name="actions">
                <a href="{{ route('vendors.create') }}"
                    class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-500 hover:to-cyan-500 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-teal-500/25">
            ...
            <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-cyan-400">{{ $stats['service'] }}</p>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <tr class="hover:bg-gray-700/30 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-10 h-10 rounded-xl bg-gradient-to-br from-teal-500 to-cyan-600 flex items-center justify-center">
                                            <span
                                                class="text-white font-bold text-sm">{{ strtoupper(substr($vendor->name, 0, 2)) }}</span>
                                        </div>
                                        <div>
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-3 py-1 rounded-full text-sm font-medium bg-{{ $vendor->type_color }}-500/20 text-{{ $vendor->type_color }}-400">
                                        {{ $vendor->type_label }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    @if($vendor->status === 'active')
                                        <span
                                            class="px-3 py-1 rounded-full text-sm font-medium bg-emerald-500/20 text-emerald-400">Aktif</span>
                                    @else
                                        <span
                                            class="px-3 py-1 rounded-full text-sm font-medium bg-gray-500/20 text-gray-400">Inactive</span>
                                    @endif
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-12 text-center text-gray-400">
                                    ...
                                    <a href="{{ route('vendors.create') }}"
                                        class="text-teal-400 hover:underline mt-2 inline-block">Tambah vendor pertama</a>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
