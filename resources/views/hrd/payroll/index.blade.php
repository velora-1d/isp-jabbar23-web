...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-gray-400 mt-1">Kelola penggajian karyawan</p>
        </div>
        <div class="flex gap-2">
            <input type="month" id="periodFilter" value="{{ $period }}" 
            ...
            <a href="{{ route('payroll.create') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25">
        ...
        <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-yellow-400">Rp {{ number_format($stats['total_amount'], 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-900/50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-right text-xs font-semibold text-gray-400 uppercase tracking-wider">Gaji Bersih</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">Kehadiran</th>
                ...
                <tbody class="divide-y divide-gray-700/50">
                    ...
                    <tr class="hover:bg-gray-700/30 transition-colors">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
                                    <span class="text-white font-bold text-sm">{{ strtoupper(substr($payroll->user->name ?? 'U', 0, 2)) }}</span>
                                </div>
                                <div>
                        ...
                        <td class="px-6 py-4">
                            <span class="px-3 py-1 rounded-full text-sm font-medium bg-{{ $payroll->status_color }}-500/20 text-{{ $payroll->status_color }}-400">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('payroll.approve', $payroll) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 bg-emerald-600/20 hover:bg-emerald-600/40 text-emerald-400 rounded-lg transition-colors" title="Setujui">
                                ...
                                <form action="{{ route('payroll.markPaid', $payroll) }}" method="POST" class="inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="inline-flex items-center gap-1 px-3 py-1.5 bg-blue-600/20 hover:bg-blue-600/40 text-blue-400 rounded-lg transition-colors" title="Bayar">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/></svg>
                                    </button>
                                </form>
                                @endif

                                <a href="{{ route('payroll.edit', $payroll) }}" class="inline-flex items-center gap-1 px-3 py-1.5 bg-amber-600/20 hover:bg-amber-600/40 text-amber-400 rounded-lg transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </a>
                                <form action="{{ route('payroll.destroy', $payroll) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data payroll ini?')">
                                    @csrf
                                    @method('DELETE')
