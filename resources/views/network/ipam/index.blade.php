            ...
            @forelse($pools as $pool)
                <div class="bg-gray-800/50 backdrop-blur-xl rounded-2xl border border-gray-700/50 p-6">
                    <div class="flex items-start justify-between mb-4">
                        ...
                        <span
                            class="px-3 py-1 rounded-full text-sm font-medium bg-{{ $pool->type_color }}-500/20 text-{{ $pool->type_color }}-400">
                            {{ ucfirst($pool->type) }}
                        </span>
                    </div>
                    ...
                    <div class="flex gap-2">
                        <form action="{{ route('network.ipam.pools.destroy', $pool) }}" method="POST" class="inline"
                            onsubmit="return confirm('Hapus pool ini? Semua IP di dalamnya akan terhapus.')">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
