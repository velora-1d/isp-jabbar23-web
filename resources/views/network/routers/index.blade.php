...
@section('content')
    <div class="space-y-6">
        ...
        <x-filter-bar :filters="$filters ?? []">
            <x-slot name="global">
                <x-filter-global :search-placeholder="'Cari Router...'" />
            </x-slot>
                    ...
                    <tbody class="divide-y divide-gray-700/50">
                        @forelse($routers as $router)
                            <tr class="hover:bg-gray-700/30 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="px-3 py-1 rounded-full text-sm font-medium bg-{{ $router->status_color }}-500/20 text-{{ $router->status_color }}-400">
                                        {{ $router->status_label }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <form action="{{ route('network.routers.sync', $router) }}" method="POST"
                                            class="inline">
                                            @csrf
                                            <button type="submit"
                                        ...
                                        <form action="{{ route('network.routers.import', $router) }}" method="POST"
                                            class="inline"
                                            onsubmit="return confirm('Import semua customer dari Mikrotik? (Ini mungkin memakan waktu)')">
                                            @csrf
                                            <button type="submit"
                                                class="inline-flex items-center gap-1 px-3 py-1.5 bg-indigo-600/20 hover:bg-indigo-600/40 text-indigo-400 rounded-lg transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <form action="{{ route('network.routers.destroy', $router) }}" method="POST"
                                            class="inline" onsubmit="return confirm('Hapus router ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center text-gray-400">
                                    ...
                                    <a href="{{ route('network.routers.create') }}"
                                        class="text-cyan-400 hover:underline mt-2 inline-block">Tambah router pertama</a>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
